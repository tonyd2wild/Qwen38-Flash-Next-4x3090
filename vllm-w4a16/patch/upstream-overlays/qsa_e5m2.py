# e5m2 VARIANT of andreasgru's PR #54846 overlay (Kai, 2026-09-06): adds the fp8_e5m2 KV path for Ampere GPUs, where Triton
# has no fp8e4nv. Everything else is byte-identical to upstream-overlays/qsa.py. Mounted only when KV_DTYPE=fp8_e5m2.
...
_QSA_KV_CACHE_DTYPES: tuple[str, ...] = ("auto", "bfloat16", "fp8", "fp8_e4m3", "fp8_e5m2", "nvfp4")  # e5m2 variant (Ampere)
...
# kernel.
_QSA_KV_STORAGE_DTYPES = (torch.bfloat16, torch.uint8, torch.float8_e4m3fn, torch.float8_e5m2)
...
class Qwen4ExpQSAFlashAttentionImpl(FlashAttentionImpl):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if self.kv_cache_dtype not in _QSA_KV_CACHE_DTYPES:
            raise NotImplementedError(
                "Qwen4Exp QSA requires a BF16, FP8-E4M3, FP8-E5M2 or NVFP4 main KV cache"
            )
    ...
    def forward_qsa(
        ...
        if self.kv_cache_fp8:
            ...
            _fp8 = torch.float8_e5m2 if self.kv_cache_dtype == "fp8_e5m2" else torch.float8_e4m3fn
            key_cache = key_cache.view(_fp8)
            value_cache = value_cache.view(_fp8)
        if query.dtype != torch.bfloat16:
            raise NotImplementedError("Qwen4Exp QSA requires BF16 queries")
        if key_cache.dtype not in (torch.bfloat16, torch.float8_e4m3fn, torch.float8_e5m2, torch.uint8):
            raise NotImplementedError(
                "Qwen4Exp QSA requires BF16, FP8-E4M3, FP8-E5M2 or NVFP4 K/V"
            )
