# e5m2 VARIANT of the ops_qsa.py overlay (Kai, 2026-09-06): accepts float8_e5m2 caches on the fp8 kernel path.
...
def qsa_sparse_paged_attention(
    ...
    assert k_cache.dtype in (torch.bfloat16, torch.float8_e4m3fn, torch.float8_e5m2, torch.uint8)
    use_fp8 = k_cache.dtype in (torch.float8_e4m3fn, torch.float8_e5m2)
    if use_nvfp4:
