...
**Count to 100, single stream, temperature 0: 194 tok/s median** (161.8 / 194.2 / 196.4), 391 tokens in 2.2 s, TTFT 160 to 200 ms. Without the draft the same box does 55.8. Context 64K with 2 seats, KV pool 186,016 tokens, 18.8 GB per card.
...
| KV cache | BF16 (the only format this model's attention can use on Ampere, see below) | |
...
LM_ONLY=1 NCCL_MODE=nvl PLE_MODE=staged GRAPHS=nocompile MTP=3 TP=4 GMU=0.97 SEQS=2 CHUNK=2048 \
MAXLEN=65536 KV_DTYPE=auto CAPTURE_SIZES=4,8 \
...
| 10 | plus language-model-only, gmu 0.97, 64K, 2 seats | 194.2 tok/s. 18.8 GB per card, pool 186,016. The serving config. |
| 11 | fp8_e5m2 KV through a variant of the attention overlay (`patch/upstream-overlays/*_e5m2.py`) | see the update at the bottom |
...
Counting to 100 is the easiest possible text for a draft (100% acceptance). Prose, code and long context will accept fewer draft tokens; the same 40-prompt harness the Spark lanes use lives in the sister repo and has not been run on this box yet. KV is BF16 at 14 KB per token per card, so the pool is small next to the Sparks (one Spark: 1.03M tokens). The box holds one model at a time: this lane and the llama.cpp lane cannot coexist with the 27B and 35B lanes.
...
- 2026-09-06 7:50 PM ET: lane added after boot 10. Boot 11 (e5m2 KV) and the TP2 x PP2 layout are in progress; results land here as they finish.
